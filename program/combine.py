...
def combine_f():
    ...
    for q_id in q:
        ...
        reading_file = os.path.join(c,str(q_id)+"_reading.avi")
        answering_file = os.path.join(c,str(q_id)+"_answering.avi")

        print("reading_file",reading_file)
        print("answering_file",answering_file)

        evaluate_video(reading_file , result, 0)
        evaluate_video(answering_file, result, 1)
        Gend_value, bal_value, pronoun_value, acc_value = testpro.speech_analysis(answering_file)
        ...
        if(q_id in l_questions_text):
            result["text"].append(text_main.text_analysis(answering_file))
